		...
		void Update () {
			...
			if (Input.GetMouseButtonDown (0)) {
				...
				if (Physics.Raycast (ray, out hit, maxRayDistance, collisionLayer)) {
                    ...
                    Debug.Log("hit: "+ hit.collider.gameObject + "with tag: "+ hit.collider.gameObject.tag);
                    if(hit.collider.gameObject.name == "Plane"){
                        GetComponent<MainManager>().ReceivedTouch(hit.point, hit.transform.rotation);    
                    }else{
                        GetComponent<MainManager>().TouchedObject(hit.collider.gameObject);    
                    }
